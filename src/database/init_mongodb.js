...
const dbName = 'afad_earthquake_data';
const collectionName = 'earthquake_records';

// Your schema validation JSON (paste your full JSON schema here exactly as you gave me)
      ...
      properties: {
        event_info: {
          ...
          properties: {
            ...
            event_date: { bsonType: "string" },
            event_time: { bsonType: "string" },
...
async function init() {
  ...
  try {
    ...
    if (collections.length === 0) {
      ...
      await db.createCollection(collectionName, schemaValidation);
      console.log('Collection created with schema validation.');
    } else {
      // Collection exists, update validation rules
      console.log(`Updating validation schema for existing collection '${collectionName}'...`);
      await db.command({
        collMod: collectionName,
        validator: schemaValidation.validator,
        validationLevel: schemaValidation.validationLevel,
        validationAction: schemaValidation.validationAction,
      });
      console.log('Schema validation updated.');
    }

    // Create indexes for faster queries (optional, adjust fields if needed)
    await db.collection(collectionName).createIndexes([
      { key: { "event_info.event_id": 1 }, name: "event_id_idx" },
      { key: { "station_info.station_code": 1 }, name: "station_code_idx" },
      { key: { "event_info.event_datetime": 1 }, name: "event_datetime_idx" }
    ]);
